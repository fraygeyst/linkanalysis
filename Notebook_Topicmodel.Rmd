...
data <- RSQLite::dbGetQuery(con, "SELECT tweet_text FROM joinedData LIMIT 60000")
...
plot.STM(model5, type ="summary", text.cex = 0.4)

```
``` {r}
plot(model5, type = "label", topics = c(1, 7, 12, 8), main = "Themenbegriffe")
...
cloud(model5, topic=1, max.words = 100)
...
```{r}
plot.STM(model5, type="perspectives", topics = c(1,7))
```
